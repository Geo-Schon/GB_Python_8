...
check_dir = 'C:\\Users\\Geo\\Desktop\\GB_Python_8'
results = walker_dir(check_dir)
print(results)

with open('second_task.json', 'w', encoding='utf-8') as js_f:
    json.dump(results, js_f, indent=2)

with open("second_task.csv", 'w', newline='', encoding='utf-8') as csv_f:
    writer = csv.DictWriter(csv_f, fieldnames=results[0].keys(), delimiter='|')
    writer.writeheader()
    writer.writerows(results)

with open("second_task.pickle", 'wb') as pcl_f:
